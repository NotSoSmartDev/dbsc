...
let g_listening_host = process.env.DBSC_HOST
  ? process.env.DBSC_HOST
  : "0.0.0.0";
...
handlebars.registerHelper(
  "hasCookieEverRefreshed",
  function (expires, expiresAtStartSession) {
...
fastify.get("/", function (request, reply) {
  ...
  return reply
    .header("Origin-Trial", g_ot_token)
...
fastify.post("/internal/StartSessionForm", function (request, reply) {
  ...
  reply.setCookie(
    "dbsc-registration-sessions-id",
    to_json(newSession.sessionId),
    {
      domain: g_listening_host,
      path: "/",
      signed: false,
  ...
  let headerStr =
    '(ES256 RS256); path="' +
    encodeURIComponent("StartSession") +
    '"; challenge="' +
    challengeStr +
    '"';
...
fastify.post("/internal/StartSession", function (request, reply) {
  ...
  if (
    sessionInfo.authCode !== decoded.authorization ||
    g_challenges[sessionInfo.challengeKey] !== decoded.jti
  ) {
    // Unauthorized error.
  ...
  return reply.code(200).send(responseStr);
});
...
fastify.listen(
  ...
  function (err, address) {
    ...
    console.log(
      `Your app is listening on ${address} with ${g_listening_port} and ${g_listening_host}`,
    );
  },
);
